    ...
    public static async Task MigrateAndSeedAsync(IServiceProvider services)
    {
        ...
        if (!await db.TaskItems.AnyAsync(t => t.ProjectId == p1.Id))
        {
            ...
            db.TaskItems.AddRange(
                ...
                new TaskItem { Title = "Build subscription management UI for customers", Description = "Create a self-service portal where customers can view their current plan, upgrade or downgrade, and download invoices without contacting support.", Status = "Ready", Priority = "Medium", EstimateMinutes = 480, ProjectId = p1.Id, EpicId = e1Bill?.Id, CreatedById = mia?.Id, AssigneeId = mia?.Id },
                new TaskItem { Title = "Implement invoice generation and PDF export", Description = "Auto-generate a PDF invoice for every successful charge including line items, tax breakdown and company branding. Store and serve via signed URLs.", Status = "Ready", Priority = "Medium", EstimateMinutes = 360, ProjectId = p1.Id, EpicId = e1Bill?.Id, CreatedById = lucas?.Id, AssigneeId = lucas?.Id },
                new TaskItem { Title = "Handle failed payment retries with exponential back-off", Description = "Implement smart retry logic for failed charges with progressive delays of 1, 3 and 7 days. Notify the customer via email at each attempt and downgrade access after final failure.", Status = "Ready", Priority = "Low", EstimateMinutes = 240, ProjectId = p1.Id, EpicId = e1Bill?.Id, CreatedById = isabelle?.Id, AssigneeId = isabelle?.Id },
                ...
                new TaskItem { Title = "Create tenant onboarding wizard", Description = "Guide new tenants through workspace setup in four steps: invite team, configure SSO, set up billing and choose a default workflow template.", Status = "Ready", Priority = "Medium", EstimateMinutes = 480, ProjectId = p1.Id, EpicId = e1Tent?.Id, CreatedById = chloe?.Id, AssigneeId = chloe?.Id },
                new TaskItem { Title = "GDPR-compliant data export and right to erasure", Description = "Allow tenant admins to request a full data export as JSON/CSV and trigger account deletion that anonymises all PII within 72 hours per GDPR Article 17.", Status = "Ready", Priority = "Medium", EstimateMinutes = 360, ProjectId = p1.Id, EpicId = e1Tent?.Id, CreatedById = james?.Id, AssigneeId = james?.Id },
                ...
                new TaskItem { Title = "Implement webhook delivery with retry and HMAC signing", Description = "Deliver events to customer endpoints with HMAC-SHA256 request signing, an exponential retry policy and a dashboard showing per-endpoint delivery health.", Status = "Ready", Priority = "High", EstimateMinutes = 480, ProjectId = p1.Id, EpicId = e1Api?.Id, CreatedById = mia?.Id, AssigneeId = mia?.Id },
        ...
        if (!await db.TaskItems.AnyAsync(t => t.ProjectId == p2.Id))
        {
            ...
            db.TaskItems.AddRange(
                ...
                new TaskItem { Title = "Add contact activity timeline view", Description = "Show a chronological timeline of all interactions with a contact: emails sent, calls logged, deals updated and notes added, with filter by activity type.", Status = "Ready", Priority = "Medium", EstimateMinutes = 480, ProjectId = p2.Id, EpicId = e2Contact?.Id, CreatedById = oliver?.Id, AssigneeId = thomas?.Id },
                new TaskItem { Title = "Merge duplicate contact detection and resolution", Description = "Run a daily background job that scores contact pairs by name and email similarity, surfaces likely duplicates in a review queue and merges them on confirmation.", Status = "Ready", Priority = "High", EstimateMinutes = 720, ProjectId = p2.Id, EpicId = e2Contact?.Id, CreatedById = priya?.Id, AssigneeId = priya?.Id },
                ...
                new TaskItem { Title = "Activity reminders and follow-up scheduling", Description = "Let reps schedule follow-up tasks linked to a deal. Send email and in-app reminders 24 hours and 1 hour before the due time with one-click snooze options.", Status = "Ready", Priority = "Medium", EstimateMinutes = 240, ProjectId = p2.Id, EpicId = e2Sales?.Id, CreatedById = sarah?.Id, AssigneeId = quirin?.Id },
                ...
                new TaskItem { Title = "Audience segmentation for targeted campaigns", Description = "Allow marketers to define campaign audiences using contact fields, tags and behaviour triggers such as last-open date or deal stage. Preview segment size before sending.", Status = "Ready", Priority = "High", EstimateMinutes = 480, ProjectId = p2.Id, EpicId = e2Email?.Id, CreatedById = amara?.Id, AssigneeId = amara?.Id },
                new TaskItem { Title = "A/B testing framework for email subject lines", Description = "Split a campaign send into two variants with different subject lines. After the test period, automatically send the winning variant to the remaining audience.", Status = "Ready", Priority = "Medium", EstimateMinutes = 720, ProjectId = p2.Id, EpicId = e2Email?.Id, CreatedById = priya?.Id, AssigneeId = lena?.Id },
                ...
                new TaskItem { Title = "Business card scanner with OCR to contact creation", Description = "Use the device camera and an OCR engine to scan printed business cards and pre-fill a new contact form with detected name, title, email and phone number.", Status = "Ready", Priority = "Low", EstimateMinutes = 360, ProjectId = p2.Id, EpicId = e2Mobile?.Id, CreatedById = sarah?.Id, AssigneeId = priya?.Id }
            );
        }
        ...
        if (!await db.TaskItems.AnyAsync(t => t.ProjectId == p3.Id))
        {
            ...
            db.TaskItems.AddRange(
                ...
                new TaskItem { Title = "Password reset with SMS OTP fallback", Description = "Send a six-digit OTP via email with an SMS fallback. Expire the token after 10 minutes and enforce a three-attempt limit before requiring a new code to be requested.", Status = "Ready", Priority = "Medium", EstimateMinutes = 360, ProjectId = p3.Id, EpicId = e3Onboard?.Id, CreatedById = oliver?.Id, AssigneeId = oliver?.Id },
                ...
                new TaskItem { Title = "Conflict resolution strategy for concurrent edits", Description = "Implement a last-write-wins strategy with server timestamp authority for most fields. For task status, present a conflict dialog when the server version differs from the local edit.", Status = "Ready", Priority = "High", EstimateMinutes = 960, ProjectId = p3.Id, EpicId = e3Offline?.Id, CreatedById = james?.Id, AssigneeId = james?.Id },
                new TaskItem { Title = "Background sync triggered on network reconnect", Description = "Listen for network state changes and trigger a sync queue flush whenever connectivity is restored. Show a subtle banner informing the user that changes are being uploaded.", Status = "Ready", Priority = "High", EstimateMinutes = 720, ProjectId = p3.Id, EpicId = e3Offline?.Id, CreatedById = noah?.Id, AssigneeId = noah?.Id },
                ...
                new TaskItem { Title = "Per-user notification preference settings screen", Description = "Let users toggle individual notification categories — task assignments, comments, due-date reminders, status changes — and configure a daily quiet-hours window.", Status = "Ready", Priority = "Medium", EstimateMinutes = 360, ProjectId = p3.Id, EpicId = e3Push?.Id, CreatedById = sophie?.Id, AssigneeId = sophie?.Id },
                ...
                new TaskItem { Title = "Automated performance regression tests in CI pipeline", Description = "Add a Detox E2E test suite measuring render times for the task list and dashboard screens. Fail the CI build if p95 frame render time exceeds 16 ms (60 fps threshold).", Status = "Ready", Priority = "Medium", EstimateMinutes = 480, ProjectId = p3.Id, EpicId = e3Perf?.Id, CreatedById = quirin?.Id, AssigneeId = diego?.Id }
            );
        }
        ...
        if (!await db.TaskItems.AnyAsync(t => t.ProjectId == p4.Id))
        {
            ...
            db.TaskItems.AddRange(
                ...
                new TaskItem { Title = "Data validation pipeline with dead-letter queue", Description = "Add a validation stage that checks required fields, value ranges and schema conformance. Route invalid records to a dead-letter queue with a rejection reason for manual review.", Status = "Ready", Priority = "High", EstimateMinutes = 720, ProjectId = p4.Id, EpicId = e4Ingest?.Id, CreatedById = isabelle?.Id, AssigneeId = isabelle?.Id },
                ...
                new TaskItem { Title = "Alert snooze and acknowledge workflow", Description = "Add snooze (15 min, 1 h, 4 h) and acknowledge actions to each alert. Acknowledged alerts suppress re-notification until the condition clears and re-triggers.", Status = "Ready", Priority = "Medium", EstimateMinutes = 240, ProjectId = p4.Id, EpicId = e4Alert?.Id, CreatedById = lucas?.Id, AssigneeId = lucas?.Id },
                new TaskItem { Title = "On-call escalation policy with rotation schedule", Description = "Define escalation tiers where an alert escalates to the next on-call engineer if not acknowledged within a configurable timeout. Integrate with a weekly rotation schedule.", Status = "Ready", Priority = "Medium", EstimateMinutes = 720, ProjectId = p4.Id, EpicId = e4Alert?.Id, CreatedById = mia?.Id, AssigneeId = mia?.Id },
